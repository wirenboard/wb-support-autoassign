  ...
  if (BRAIN === 'ai' && candidates.length) {
    try {
      ...
      if (ai === null) {
        ...
        // Sol сказал «подходящих нет» — НЕ откладываем тему, а откатываемся на правила
        // (они назначат по загрузке онлайновому). decision оставляем null → сработает блок правил ниже.
        log(`#${id} ИИ: подходящих нет (${ai.reason ?? ''}) — откат на правила`);
      }
    } catch (e) {
