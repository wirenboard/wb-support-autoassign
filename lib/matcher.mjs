...
// Персональные рабочие часы инженера: строка "HH:MM-HH:MM" по МСК (engineers.<u>.hours).
// null/пусто → без ограничения по часам. nowMin — минуты от полуночи МСК;
// null → фильтр по часам отключён (офлайн-тест / --force-hours).
function withinHours(hours, nowMin) {
  if (!hours || nowMin == null) return true;
  const m = /^(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})$/.exec(String(hours).trim());
  if (!m) return true;                       // кривой формат — не режем
  const start = +m[1] * 60 + +m[2], end = +m[3] * 60 + +m[4];
  return nowMin >= start && nowMin < end;
}

// Онлайн-кандидаты после жёстких гейтов: EN-тройка / auto+!away, «в сети» И в своих часах.
// Общие рамки для ИИ-оператора и для правил.
export function poolFor(cfg, presence, isEn, nowMin = null) {
  const limit = cfg.presence.online_within_minutes;
  const base = isEn
    ? cfg.gates.english.engineers
    : Object.entries(cfg.engineers).filter(([, e]) => e.auto).map(([u]) => u);
  return base
    .map(u => ({ u, ...(presence[u] ?? { minutes_since_seen: 1e9, load: 0, paused: false }) }))
    .filter(c => !cfg.engineers[c.u]?.away)   // отпуск (флаг away в routing.yaml) — оба пула
    .filter(c => !c.paused)                    // само-пауза: инженер в группе autoassign-paused
    .filter(c => c.minutes_since_seen <= limit)
    .filter(c => withinHours(cfg.engineers[c.u]?.hours, nowMin));
}
...
export function pick(cfg, presence, domain, isEn, nowMin = null) {
  const online = poolFor(cfg, presence, isEn, nowMin).map(c => ({
    ...c,
    skill: domain ? (cfg.skills[domain]?.[c.u] ?? null) : null,
  }));
  // ранжируем по виртуальной загрузке (effLoad = load + load_bias), чтобы load_bias
  // реально понижал приоритет инженера; при равенстве — кто раньше был онлайн.
  ...
  if (domain) {
    ...
    // компетентных в сети нет, но кто-то онлайн — назначаем по загрузке (без «ручного разбора»)
    if (online.length) return { pick: rank(online)[0], tier: 'по загрузке (без профиля)', cand: rank(online) };
    return { pick: null, tier: 'никого в сети', cand: [] };
  }
  // домен не определён — тоже назначаем по загрузке тому, кто онлайн
  if (online.length) return { pick: rank(online)[0], tier: 'по загрузке', cand: rank(online) };
  return { pick: null, tier: 'никого в сети', cand: [] };
}
